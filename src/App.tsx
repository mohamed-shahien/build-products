...
import { ErrorMessag } from './components/Error'
...
function App() {
  ...
  const renderFormInputList = formInputsList.map(input => (
    <div className='flex flex-col' key={input.id}>
      ...
      <ErrorMessag msg={errors[input.name]}/>
    </div>
  ))

  return (
    <main className='container mx-auto'>
      <Buttom className="bg-indigo-700 hover:bg-indigo-800" onClick={openModal}>Add</Buttom>
    ...
    <Modal isOpen={isOpen} closeModal={closeModal} title='Add New Product'>
      <form className="space-y-3" onSubmit={submitHandler}>
        ...
        <div className="flex items-center space-x-3">
          <Buttom className="bg-indigo-700 hover:bg-indigo-800">Submet</Buttom>
          <Buttom className="bg-gray-500 hover:bg-gray-700" onClick={onCansel}>Cancel</Buttom>
        </div>
      </form>
    </Modal>
    </main>
